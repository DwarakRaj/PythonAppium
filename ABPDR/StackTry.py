...
blocking_time_duration = ["Total Blocking Time", float(blocking_time_duration.replace("\xa0ms", ""))]
...
# https://stackoverflow.com/questions/72119006/trying-to-pull-google-lighthouse-info-in-a-scorecard

driver = webdriver.Chrome()
driver.get('https://googlechrome.github.io/lighthouse/scorecalc/#FCP=' + str(list[0][1]) + '&SI=' + str(
    list[1][1]) + '&LCP=' + str(list[4][1]) + '&TTI=' + str(list[2][1]) + '&TBT=' + str(list[3][1]) + '&CLS=' + str(
    list[5][1]) + '&device=' + device + '&version=6')

time.sleep(3)
#element = driver.find_element_by_class_name("lh-gauge__percentage")
#Please use find_element(by=By.CLASS_NAME, value=name)

element = driver.find_element(By.CLASS_NAME, value="lh-gauge__percentage")
...
for x in range(numLoops):
    ...
    print("\nWoking on " + str(
        list[differenceslist.index(max(differenceslist))][0]) + " would improve performance " + str(
        max(differenceslist)) + " points.")
for x in range(len(differenceslist)):
    print("If you improve 50% of " + str(list[x][0]) + " you will improve " + str(
        differenceslist[x]) + " points your overall score")